...
distance_to_camera_mm = 700  # 50 см = 500 мм
...
# Функция для определения наклона головы
def get_head_rotation(landmarks):
	# Используем ключевые точки для вычисления наклона головы (например, нос, уши)
	nose = landmarks.landmark[1]
	left_eye = landmarks.landmark[33]
	right_eye = landmarks.landmark[263]

	# Простой расчет угла наклона головы относительно камеры
...
# Функция для применения сглаживания (скользящее среднее)
def smooth_angles(angles, window_size=5):
	"""Применяет сглаживание с помощью скользящего среднего."""
	smoothed = np.convolve(angles, np.ones(window_size) / window_size, mode='valid')
	return smoothed
...
while cap.isOpened():
	...
	if result.multi_face_landmarks:
		for face_landmarks in result.multi_face_landmarks:
			# Получение угла наклона головы
			head_rotation = get_head_rotation(face_landmarks)

			# Координаты для левого и правого глаза
			...
			left_angle = calculate_angle_with_distance(left_eye_inner_px, left_eye_outer_px, left_pupil_px,
			                                           distance_to_camera_mm)
			right_angle = calculate_angle_with_distance(right_eye_inner_px, right_eye_outer_px, right_pupil_px,
			                                            distance_to_camera_mm)

			# Коррекция углов с учетом наклона головы
			left_angle -= head_rotation
			right_angle -= head_rotation
...
# Применение сглаживания (скользящее среднее)
left_eye_angles_smoothed = smooth_angles(left_eye_angles)
right_eye_angles_smoothed = smooth_angles(right_eye_angles)

# Построение и сохранение графика
plt.figure(figsize=(12, 6))
plt.plot(time_data[2:-2], left_eye_angles_smoothed, label='Left Eye Angle (Smoothed)',
         color='green')  # Для сглаженного графика
plt.plot(time_data[2:-2], right_eye_angles_smoothed, label='Right Eye Angle (Smoothed)',
         color='blue')  # Для сглаженного графика
plt.axhline(0, color='black', linestyle='--', linewidth=0.8)
plt.xlabel('Time (s)')
plt.ylabel('Eye Angle (degrees)')
plt.legend()
plt.title('Smoothed Eye Angle Over Time')
plt.grid(alpha=0.3)
plt.savefig('Angles_video4_smoothed.png', dpi=300)
