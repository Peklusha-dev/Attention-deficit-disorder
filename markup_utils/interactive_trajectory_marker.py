...
# Настройки
IMAGE_FOLDER = None
VIDEO_PATH = "../dataset/eyes_movies/маятник.MOV"
OUTPUT_PUPIL_CSV = "../маятник.csv"
OUTPUT_ANNOTATIONS_CSV = "../маятник_annotations.csv"  # Изменил имя файла для аннотаций
DETECTOR_TYPE = "head_compensated"
...
def safe_get_event_data(event, default_type='unknown', default_start=0, default_end=1):
    """Безопасное получение данных события."""
    try:
        event_type = event.get('type', default_type)
        start_time = float(event.get('start_time', default_start))
        end_time = float(event.get('end_time', default_end))

        # Убедимся, что времена корректны
        if start_time >= end_time:
            end_time = start_time + 0.1

        return event_type, start_time, end_time
    except (ValueError, TypeError):
        return default_type, default_start, default_end
...
def create_trajectory_plot(df, annotations_df=None):
    ...
    if annotations_df is not None and len(annotations_df) > 0:
        for idx, event in annotations_df.iterrows():
            event_type, start_time, end_time = safe_get_event_data(event)
            ...
            try:
                ...
                print(f"Ошибка при добавлении аннотации {idx}: {e}")
                continue
...
try:
    ...
    fig = create_trajectory_plot(df, pd.DataFrame())  # Создаем пустой график без аннотаций
...
def update_plot(save_clicks, delete_clicks, export_clicks, event_type, selected_data):
    ...
    if button_id == 'save-button' and selected_data:
        ...
        try:
            ...
            message = html.Div(f"Ошибка при сохранении: {e}", style={'color': 'red'})
...
def create_annotations_list():
    ...
    for idx, row in annotations_df.iterrows():
        event_type, start_time, end_time = safe_get_event_data(row)
        color = 'red' if event_type == 'saccade' else 'green'
        items.append(html.Div([
            html.Span(f"{event_type}: ", style={'font-weight': 'bold', 'color': color}),
            html.Span(f"{start_time:.3f}с - {end_time:.3f}с")
